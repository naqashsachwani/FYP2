...
const ProductDescription = ({ product }) => {
    ...
    // Use product.ratings (plural) to match your schema
    const [localReviews, setLocalReviews] = useState(product?.ratings || [])

    const handleReviewSuccess = (newReview) => {
        setLocalReviews([newReview, ...localReviews])
        router.refresh() // Refresh server data to update average rating & count
    }
            ...
            <div className="flex border-b border-slate-200 mb-6 max-w-2xl">
                {['Description', 'Reviews'].map((tab, index) => (
                    <button
                        ...
                        className={`${
                            tab === selectedTab
                                ? 'border-b-[1.5px] border-emerald-500 text-emerald-600 font-semibold'
                                : 'text-slate-400'
                            ...
                            localReviews.map((item, index) => (
                                <div key={index} className="flex gap-5 mb-8">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm max-w-lg my-3 leading-relaxed text-slate-600">
                                            {item.review}
                                        </p>
            ...
            <div className="flex gap-3 mt-14 pt-8 border-t border-slate-100">
                ...
                <div>
                    ...
                    <Link
                        ...
                        className="flex items-center gap-1.5 text-emerald-500 hover:text-emerald-600 transition font-medium mt-1"
                    >
