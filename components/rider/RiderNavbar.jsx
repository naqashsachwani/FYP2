...
import { Menu, Home, Settings, LogOut, ShieldCheck, Bell, X, Store } from "lucide-react" 
...
export default function RiderNavbar({ onToggleSidebar, isSidebarOpen }) {
  ...
  return (
    <header className="h-[73px] shrink-0 flex items-center justify-between px-4 lg:px-6 bg-white border-b border-slate-200 z-50">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 flex flex-col z-40 overflow-hidden">
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <div key={n.id} onClick={(e) => markAsRead(n.id, e)} className={`p-3 border-b cursor-pointer ${!n.isRead ? 'bg-blue-50' : 'bg-white'}`}>
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-3 bg-slate-50 rounded-full pl-2 pr-4 py-1.5 shadow-sm border border-slate-100 hover:bg-slate-100 transition-all"
          >
            <div className="w-8 h-8 rounded-full overflow-hidden border border-blue-200 bg-white">
              <img src={user?.imageUrl} alt="Profile" className="w-full h-full object-cover" />
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-sm font-bold text-slate-700 leading-tight">Hi, {user?.firstName}</p>
              <p className="text-[10px] text-slate-500 font-medium">RIDER</p>
            </div>
          </button>
            ...
            <>
              ...
              <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-40 overflow-hidden">
                <div className="px-2 space-y-1">
                  ...
                  <Link href="/" className="flex items-center gap-3 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-lg font-medium">
                    <Home size={16} className="text-slate-500" /> Go to Homepage
                  </Link>
                  
                  {showAdminBtn && (
                    <Link href="/admin" className="flex items-center gap-3 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-lg font-medium">
                      <ShieldCheck size={16} className="text-slate-500" /> Admin Dashboard
                    </Link>
                  )}
                  
                  {(showAdminBtn || showSellerBtn) && (
                    <Link href="/store" className="flex items-center gap-3 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-lg font-medium">
                      <Store size={16} className="text-slate-500" /> Store Dashboard
                    </Link>
                  )}

                  <button onClick={() => openUserProfile()} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 rounded-lg font-medium">
