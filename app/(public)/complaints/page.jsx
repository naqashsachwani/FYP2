...
export default function UserComplaintsPage() {
  ...
  // ✅ FIXED FILTER LOGIC: Combine OPEN and IN_PROGRESS together
  const filteredComplaints = useMemo(() => {
    return complaints.filter((comp) => {
      if (filterStatus === "ALL") return true;
      
      // If user selects "In Progress", show both OPEN and IN_PROGRESS tickets
      if (filterStatus === "IN_PROGRESS") {
        return comp.status === "OPEN" || comp.status === "IN_PROGRESS";
      }
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className="text-xs text-gray-400 flex items-center gap-1">
                          <Clock size={14} /> {new Date(comp.createdAt).toLocaleDateString('en-GB')}
                        </span>
                      </div>
                      <div>
                      ...
                      <div className="flex flex-wrap gap-3 mt-4">
                          ...
                          {comp.imageUrl && (
                              <a href={comp.imageUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-50 rounded-lg text-xs font-medium text-indigo-600 border border-indigo-100 hover:bg-indigo-100 transition">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-slate-200 bg-white px-3 py-2 rounded-xl focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all outline-none text-slate-800 text-sm"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full border border-slate-200 bg-white px-3 py-2 rounded-xl focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all outline-none text-sm appearance-none text-slate-800"
                  >
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="w-full border border-slate-200 bg-white px-3 py-2 rounded-xl flex justify-between items-center hover:border-slate-300 transition-all text-left text-sm"
                  >
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full border border-slate-200 bg-white px-3 py-2 rounded-xl focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all outline-none resize-none text-sm text-slate-800"
                />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1.5 tracking-wider">Evidence Image (Optional)</label>
                <div className={`border-2 border-dashed rounded-xl p-3 text-center transition-colors ${selectedFile ? 'border-green-300 bg-green-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                  <input 
                    type="file" 
                    id="imageUpload"
                    accept="image/*"
                  ...
                  <label htmlFor="imageUpload" className="cursor-pointer flex flex-col items-center justify-center w-full h-full">
                    {selectedFile ? (
                      <>
                        ...
                        <span className="text-sm font-bold text-green-700 truncate w-full px-4">{selectedFile.name}</span>
                      ...
                      <>
                        ...
                        <span className="text-sm font-bold text-green-600 hover:text-green-700 transition">Click to Upload Image</span>
