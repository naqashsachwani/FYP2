...
import { Loader2, Wallet, ArrowDownLeft, ArrowUpRight, History, X, Building } from "lucide-react";
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden my-8">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h3 className="font-bold text-xl text-gray-900 flex items-center gap-2">
                <Building className="text-green-600" /> Request Withdrawal
              </h3>
              <button onClick={() => setIsWithdrawModalOpen(false)} className="text-gray-400 hover:text-gray-700 hover:bg-gray-100 p-2 rounded-full transition"><X size={20} /></button>
            </div>
            
            <form onSubmit={handleWithdraw} className="p-6 space-y-4">
              <div className="bg-green-50 text-green-800 p-4 rounded-xl text-sm border border-green-100 flex items-center justify-between font-medium">
                <span>Available Balance:</span>
                <span className="font-mono font-bold text-lg">Rs {Number(walletData.balance).toLocaleString()}</span>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Amount to Withdraw (Rs)</label>
                <input 
                  type="number" 
                  required
                  min="500"
                  ...
                  className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 focus:ring-green-500 outline-none font-mono"
                />
              </div>

              {/* ✅ CHANGED: Hardcoded to Bank Transfer and made read-only visually */}
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Transfer Method</label>
                <div className="w-full border border-gray-200 bg-gray-50 text-gray-600 p-3 rounded-xl font-medium cursor-not-allowed">
                  Bank Transfer
                </div>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Account Title / Name</label>
                <input 
                  type="text" 
                  required
                  value={accountName}
              ...
              {/* ✅ CHANGED: Placeholder tailored specifically to IBANs */}
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Account Number / IBAN</label>
                <input 
                  type="text" 
                  required
                  value={accountNumber}
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setIsWithdrawModalOpen(false)} className="flex-1 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition">Cancel</button>
                <button type="submit" disabled={isProcessing} className="flex-1 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition flex justify-center items-center gap-2 disabled:opacity-50">
